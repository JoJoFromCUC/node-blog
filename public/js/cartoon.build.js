...
//随机运动小球
function drawRandomBalls(mycanvas,num,flag){//flag表示是否连线
    var cxt = mycanvas.getContext('2d'),
        balls = [],
    ...
    for(var j=0;j<num;j++){
         aBall = {
                ...
                color: randomColor(),
                vx: (Math.random()*2-1)*2+0.5,
                vy: (Math.random()*2-1)*2+0.5
                };
        balls[j] = aBall;
    }
...
//粒子系统
function drawParticle(mycanvas,x,y,radius,fillColor){
    var ctx = mycanvas.getContext('2d');
    ctx.beginPath();
...
function jetParticles(mycanvas,num,radius,color,turbulence,life,obj){
    ...
    var tock = setInterval(function(){
        ...
        drawParticle(mycanvas,obj.x,obj.y,obj.radius,color);
